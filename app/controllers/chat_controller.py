...
def chat_with_llm(request: chat_schema.ChatRequest, db: Session = Depends(get_db), current_user: user_schema.User = Depends(verify_access_token)):
    session_id = request.session_id if request.session_id else str(uuid.uuid4())
    response = chat_service.chat_with_llm(request.query, request.company, session_id, db, current_user)
    return success_response(data=response)

@router.post("/upload-document", response_model=response_schema.SingleResponse[dict])
def upload_document(company: str, document_url: Optional[str] = None, file: Optional[UploadFile] = File(None), db: Session = Depends(get_db), current_user: user_schema.User = Depends(verify_access_token)):
    ...
    if file:
        chat_service.process_and_store_document(file, company, db, current_user)
    else:
